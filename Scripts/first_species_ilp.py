...
#       to a perfect interval) are to be avoided, especially in the outer voices."
# Semantic: if CF moves up (CfUp[t]=1) and CP also moves up (up[t]=1),
#           then CP must not arrive at a perfect consonance h[t+1,i]=0.
#           Encoded as: h[t+1,i] ≤ 1 - up[t] when CF is ascending (active only
#           when CfUp[t]=1, since otherwise the constraint is trivially satisfied).
# STATUS: ✓ Correctly encodes Fux's hidden fifths/octaves prohibition.
for t in T1:
    if CfUp[t]:
        for i in [0, 7, 12]:
