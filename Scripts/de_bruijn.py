...
def build_debruijn_graph(kgrams):
    ...
    for (
        ...
        kgrams
    ):  # breaks it into prefix and suffix and puts them in the graph with directed edge from prefix to suffix (potentially overlapping)
        notes = kgram.split()
...
def eulerian_path(graph, start):
    ...
    while stack:
        ...
        if graph[
            node
        ]:  # if it has an outgoing edge, follow it and remove it from the graph
            next_node = graph[node].pop()
            stack.append(next_node)
        else:
            path.append(
                stack.pop()
            )  # if no edges are left, we're done with this node and we add it to the final path then backtrack

    return path[
        ::-1
    ]  # need it reveresed bc we add nodes to the path when we backtrack, so the path is built in reverse order


def reconstruct_string(
    path,
):  # nodes overlap by k-1 characters so when we print it out, we should only print the new character each node adds
    result = list(path[0])
    for node in path[1:]:
...
kgrams = ["C4 E4 G4", "E4 G4 B4", "G4 B4 D5", "B4 D5 G5"]
